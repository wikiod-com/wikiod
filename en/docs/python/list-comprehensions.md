...
Given a [list comprehension][1] you can append one or more `if` conditions to filter values.
...
Note that this is quite different from the `... if ... else ...` conditional expression (sometimes known as a [ternary expression][2]) that you can use for the `<expression>` part of the list comprehension. Consider the following example:
...
If you are using Python 2.7, `xrange` may be better than `range` for several reasons as described in the [`xrange` documentation][3].
...
*See also: [Filters](https://www.wikiod.com/python/filter), which often provide a sufficient alternative to conditional list comprehensions.*


  [1]: https://docs.python.org/3/tutorial/datastructures.html#list-comprehensions
  [2]: https://www.wikiod.com/python/conditionals#Conditional Expression (or "The Ternary Operator")
  [3]: https://docs.python.org/2/library/functions.html#xrange

## List Comprehensions with Nested Loops
[List Comprehensions][1] can use nested `for` loops. You can code any number of nested for loops within a list comprehension, and each `for` loop may have an optional associated `if` test. When doing so, the order of the `for` constructs is the same order as when writing a series of nested `for` statements. The general structure of list comprehensions looks like this:
...
For the sake of readability, however, you should consider using traditional *for-loops*. This is especially true when nesting is more than 2 levels deep, and/or the logic of the comprehension is too complex. multiple nested loop list comprehension could be error prone or it gives unexpected result.
  ...
  [1]: https://docs.python.org/3/tutorial/datastructures.html#list-comprehensions
  [2]: https://ideone.com/R7uwEP
  [3]: https://ideone.com/kPO2Zy


## Refactoring filter and map to list comprehensions
The `filter` or `map` functions should often be replaced by [list comprehensions][1].  Guido Van Rossum describes this well in an [open letter in 2005][2]:
...
For iterating more than two lists simultaneously within *list comprehension*, one may use [`zip()`][1] as:
