...
[Regular expressions](https://www.wikiod.com/regex) are combinations of characters that are interpreted as rules for matching substrings. For instance, the expression `'amount\D+\d+'` will match any string composed by the word `amount` plus an integral number, separated by one or more non-digits, such as:`amount=100`, `amount is 3`, `amount is equal to: 33`, etc.   
...
Matching is done from the start of the string only. If you want to match anywhere use [`re.search`][1] instead:
```
...
```
  ...
  [1]: https://www.wikiod.com/python/regular-expressions-regex#Searching
...
Compiling a pattern allows it to be reused later on in a program. However, note that Python caches recently-used expressions ([docs][1], [SO answer](http://stackoverflow.com/a/452143/1240268)), so *"programs that use only a few regular expressions at a time needn’t worry about compiling regular expressions"*.
...
For the complete list of all available flags check the [docs][1]
...
> Note that the (?x) flag changes how the expression is parsed. It should be used first in the expression string, or after one or more whitespace characters. If there are non-whitespace characters before the flag, the results are undefined.
  ...
  [1]: https://docs.python.org/2/library/re.html#module-contents
  [2]: https://docs.python.org/2/library/re.html#regular-expression-syntax
...
[1]: https://docs.python.org/2/library/re.html#re.MatchObject.group
