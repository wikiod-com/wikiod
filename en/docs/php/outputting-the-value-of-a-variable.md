...
[`print_r`][print-r] will output a human readable format of an array or object.
...
## [`var_dump()`][var-dump] - Output human-readable debugging information about content of the argument(s) including its type and value

The output is more detailed as [compared][var_dump_vs_print_r] to `print_r` because it also outputs the **type** of the variable along with its **value** and other information like object IDs, array sizes, string lengths, reference markers, etc.

You can use [`var_dump`][var-dump] to output a more detailed version for debugging.
...
**Note**: If you are using xDebug in your development environment, the output of var_dump is limited / truncated by default. See the [official documentation][1] for more info about the options to change this.

-----------------------------------------------------------

## [`var_export()`][var-export] - Output valid `PHP` Code

[`var_export()`][var-export] dumps a `PHP` parseable representation of the item.
...
[`sprintf`][sprintf] will **return** the formatted string
