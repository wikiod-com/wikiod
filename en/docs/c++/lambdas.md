...
A **lambda expression** provides a concise way to create simple function objects. A lambda expression is a prvalue whose result object is called [closure object][1], which behaves like a function object.
...
`[]` is the **capture list**. By default, variables of the enclosing scope cannot be accessed by a lambda. *Capturing* a variable makes it accessible inside the lambda, either [as a copy][2] or [as a reference][3]. Captured variables become a part of the lambda; in contrast to function arguments, they do not have to be passed when calling the lambda.
...
A lambda expression's result object is a [closure][1], which can be called using the `operator()` (as with other function objects):
...
<!-- end version if -->
  ...
  [2]: https://www.wikiod.com/docs/c%2b%2b/572/lambdas/1856/capture-by-value#t=201607271323349545754
  [3]: https://www.wikiod.com/docs/c%2b%2b/572/lambdas/1951/capture-by-reference#t=201607271323349545754
...
Generic lambdas can take arguments by reference as well, using the usual rules for `auto` and `&`. If a generic parameter is taken as `auto&&`, this is a [*forwarding* reference][1] to the passed in argument and not an [*rvalue* reference][2]:
...
Note that `std::function<>` requires that the values stored be copyable. You can write your own [move-only-requiring `std::function`](https://www.wikiod.com/docs/c%2b%2b/2872/type-erasure/18042/a-move-only-stdfunction#t=201608061624094409548), or you could just stuff the lambda into a `shared_ptr` wrapper:
...
Lambda functions in C++ are syntactic sugar that provide a very concise syntax for writing [functors][1]. As such, equivalent functionality can be obtained in C++03 (albeit much more verbose) by converting the lambda function into a functor:
...
If the lambda function is `mutable` then make the functor's call-operator non-const, i.e.:
  ...
  [1]: https://www.wikiod.com/docs/c%2B%2B/1412/class-functors#t=201607221029133733631
