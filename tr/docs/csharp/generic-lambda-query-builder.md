---
title: "Genel Lambda Sorgu Oluşturucu"
slug: "genel-lambda-sorgu-olusturucu"
draft: false
images: []
weight: 9976
type: docs
...
Sınıf, 'ExpressionBuilder' olarak adlandırılır. Üç özelliği vardır:

     private static readonly MethodInfo ContainsMethod = typeof(string).GetMethod("Contains", new[] { typeof(string) });
     private static readonly MethodInfo StartsWithMethod = typeof(string).GetMethod("StartsWith", new[] { typeof(string) });
     private static readonly MethodInfo EndsWithMethod = typeof(string).GetMethod("EndsWith", new[] { typeof(string) });

Lambda ifadesini döndüren bir genel yöntem "GetExpression" ve üç özel yöntem:
 

- `İfade GetExpression<T>`
- `BinaryExpression GetExpression<T>`
- `ConstantExpression GetConstant`

Tüm yöntemler örneklerde ayrıntılı olarak açıklanmıştır.

## QueryFilter sınıfı
Bu sınıf yüklem filtre değerlerini tutar.
...
İşlem değerlerini tutmak için enum:
...
## GetExpression Özel aşırı yükleme
# Bir filtre için:
Sorgunun oluşturulduğu yer burasıdır, bir ifade parametresi ve bir filtre alır.
...
# İki filtre için:
Basit İfade yerine BinaryExpresion örneğini döndürür.
...
## ConstantExpression Yöntemi
"ConstantExpression", "MemberExpression" ile aynı türde olmalıdır. Bu örnekteki değer, "ConstantExpression" örneğini oluşturmadan önce dönüştürülen bir dizedir.
...
## Kullanım
Koleksiyon<QueryFilter> filtreleri = yeni Liste<QueryFilter>();
...
Bu durumda, adında "Burger" ile başlayan tüm yiyecekleri bulmak isteyen Food varlığına yönelik bir sorgudur.
